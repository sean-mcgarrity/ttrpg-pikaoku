<script lang="ts">
  ...
  import { User, PencilRuler, StepBack, ArrowLeft } from 'lucide-svelte';

  $: campaign = getCampaignFromPage();
</script>
...
<div class="antialiased flex flex-col font-sans text-white w-full bg-campaign" id="campaign-page">
  ...
  >
    ...
    <button
      class="bg-campaign text-white p-2 md:rounded-tl flex flex-row items-center gap-2 hover:brightness-125 transition-all duration-150 cursor-pointer hover:scale-105 w-full md:w-auto"
    >
      <User />
      Jizzlord
    </button>
